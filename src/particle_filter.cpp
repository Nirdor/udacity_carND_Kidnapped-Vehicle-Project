...
void ParticleFilter::prediction(double delta_t, double std_pos[], double velocity, double yaw_rate) {
  ...
  for (int i = 0; i < num_particles; ++i) {
    ...
    if (abs(yaw_rate) < 0.000001) {
      particles[i].x = particles[i].x + velocity * sin(particles[i].theta) * delta_t + dist_x(gen);
      particles[i].y = particles[i].y + velocity * cos(particles[i].theta) * delta_t + dist_y(gen);
    } else {
      ...
      particles[i].theta = particles[i].theta + yaw_rate * delta_t + dist_t(gen);
    }
  }
